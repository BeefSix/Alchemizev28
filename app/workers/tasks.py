...
async def _async_videoclip_upload_job(job_id: str, user_id: int, video_path: str, add_captions: bool, aspect_ratio: str, platforms: list[str]):
    ...
    try:
        ...
        if result.returncode != 0:
            raise ValueError(f"FFmpeg audio extraction failed. STDERR: {result.stderr}")
        ...
        total_clips_to_make = len(moments)
        clips_made = 0
        ...
        for i, moment in enumerate(moments):
            ...
            try:
                # **FIX:** Pass the user-selected aspect_ratio directly.
                clip_result = video_engine.process_single_clip(
                    video_path, moment, 
                    {"add_captions": add_captions, "aspect_ratio": aspect_ratio, "platform": "multi-platform"},
                    user_id, clip_id, full_words_data
                )
                if clip_result.get('success'):
                    # Add the generated clip to all selected platform lists
...
async def _async_content_repurpose_job(job_id: str, user_id: int, content_input: str, platforms: list[str], tone: str, style: str, additional_instructions: str):
    ...
    try:
        ...
        # **FEATURE:** Build the dynamic generation prompt
        generation_prompt = f"""
