...
def create_ass_file(words_data, output_path, start_offset):
    """Creates an ASS subtitle file with word-by-word karaoke style."""
    ...
    # Style: Yellow text with a black outline/shadow, Impact font
    style_values = f"Style: Default,Impact,48,&H00FFFF,&H000000FF,&H00000000,&H99000000,-1,0,0,0,100,100,0,0,1,2,2,2,10,10,20,1"
    event_header = "[Events]"
    event_format = "Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text"

    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("[Script Info]\nTitle: Generated by Alchemize\n\n")
        f.write(f"{style_header}\n{style_format}\n{style_values}\n\n")
        f.write(f"{event_header}\n{event_format}\n")

        # Group words into a single line for the clip
        full_sentence = " ".join([word['word'] for word in words_data])
        if not words_data:
            return

        clip_start_time = words_data[0]['start'] - start_offset
        clip_end_time = words_data[-1]['end'] - start_offset

        start_str = f"{int(clip_start_time // 3600)}:{int((clip_start_time % 3600) // 60):02}:{clip_start_time % 60:05.2f}"
        end_str = f"{int(clip_end_time // 3600)}:{int((clip_end_time % 3600) // 60):02}:{clip_end_time % 60:05.2f}"

        # Create the karaoke effect using ASS tags
        karaoke_text = ""
        for word in words_data:
            # Duration of the highlight in milliseconds
            duration_ms = int((word['end'] - word['start']) * 100) 
            karaoke_text += f"{{\\k{duration_ms}}}{word['word']} "

        dialogue_line = f"Dialogue: 0,{start_str},{end_str},Default,,0,0,0,,{karaoke_text.strip()}"
...
def process_single_clip(
    ...
    """Processes a single video clip with karaoke-style captions."""
    ...
    try:
        ...
        extract_cmd = [
            'ffmpeg', '-hwaccel', 'cuda', '-ss', str(start_time), '-i', source_video_path,
            '-t', str(duration), '-c:v', 'h264_nvenc', '-preset', 'p5', '-cq', '24',
        ...
        if add_captions and words_data:
            ...
            if clip_words:
                ...
                create_ass_file(clip_words, ass_path, start_offset=clip_start_time)
                # Use the 'ass' filter for advanced subtitles
                filter_parts.append(f"ass={ass_path}")

        # Step 3: Apply filters
        if filter_parts:
            process_cmd = [
                'ffmpeg', '-hwaccel', 'cuda', '-i', temp_clip_path, '-vf', ",".join(filter_parts),
                '-c:v', 'h264_nvenc', '-preset', 'p5', '-cq', '24', '-c:a', 'copy', '-y', final_clip_path
        ...
        return {'success': False, 'error': f"FFmpeg failed: {e.stderr[:500]}"}
    except Exception as e:
