...
from ..core.network_security import get_network_config
...
def scrape_url(url: str):
    """Securely scrape URL content with domain restrictions"""
    try:
        # Get network security configuration
        network_config = get_network_config()
        
        # Validate URL against security policy
        is_valid, error_msg = network_config.validate_url(url)
        if not is_valid:
            logger.warning(f"URL blocked by security policy: {url} - {error_msg}")
            return f"Error: URL not allowed - {error_msg}"
        
        # Use secure requests configuration
        request_config = network_config.get_requests_security_config()
        ...
        for el in soup(["script", "style", "nav", "footer", "aside", "form", "button", "header", "img", "svg", "iframe", "object", "embed"]):
            el.decompose()
...
def download_media(url: str, is_video: bool):
    """Securely download YouTube media with domain restrictions"""
...
def transcribe_local_audio_file_sync(audio_path: str, user_id: int, job_id: str):
    ...
    try:
        ...
        # Transcribe with OpenAI with timeout handling
        try:
...
def validate_video_request(video_url: str):
    """Validate video URL and duration with security checks"""
    try:
        # Get network security configuration
        network_config = get_network_config()
        
        # Validate URL against security policy
        is_valid, error_msg = network_config.validate_url(video_url)
        if not is_valid:
            return False, f"URL not allowed - {error_msg}"
        
        if not is_youtube_url(video_url):
            return False, "Not a valid YouTube URL."
        
        # Use secure yt_dlp configuration for validation
        ydl_opts = network_config.get_yt_dlp_security_config()
        ydl_opts.update({'extract_flat': False})  # Need full info for duration
