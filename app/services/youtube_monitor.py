...
class YouTubeDownloadMonitor:
    ...
    def reset_daily_stats(self):
        self.stats.clear(); self.failures = []; self.last_reset = datetime.now()
    ...
    def get_best_method(self) -> str:
        methods = ['standard_download', 'selenium_assisted']
        best_method = 'standard_download'
        best_rate = -1.0
        
        for method in methods:
            rate = self.get_success_rate(method)
            if self.stats[f'{method}_attempts'] > 3 and rate > best_rate:
                best_rate = rate
                best_method = method
        return best_method
